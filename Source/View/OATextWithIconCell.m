...
- (id)init {
	...
	[self setTextKey:OATextWithIconCellStringKey];
	[self setImageKey:OATextWithIconCellImageKey];
...
- (NSSize) cellSize; { //this doesn't seem to get called at all
...
#define _calculateDrawingRectsAndSizes \
	...
	if (_oaFlags.imagePosition == NSImageLeft) { \
		...
		rectEdge =  NSMaxXEdge; \
		if (icon == nil) imageSize = NSZeroSize; \
		else imageSize = [icon size]; \
	} \
	\
	NSRect cellFrame = aRect, ignored; \
	if (imageSize.width > 0) /* space the image away from the edge */ \
	...
	\
	if (imageSize.width > 0) /* space the image from the text */ \
		NSDivideRect(textRect, &ignored, &textRect, BORDER_BETWEEN_IMAGE_AND_TEXT, rectEdge); \
	\
	textRect = NSInsetRect(textRect, 0, (NSHeight(aRect) - [self cellSizeForBounds:aRect].height) / 2.0); /* center the text */
...
- (void) drawInteriorWithFrame:(NSRect)aRect inView:(NSView *)controlView {
	...
	// I'm not dealing with a flipped origin right now, so I'm not sure what this does
	...
	[[self icon] setFlipped:wasFlipped]; //restore previous flipped
}
...
- (void) setObjectValue:(id)obj {
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSAttributedString class]]) {
		...
		[super setObjectValue:[obj valueForKey:[self textKey]]];
		[self setIcon:[obj valueForKey:[self imageKey]]];
	}
}
